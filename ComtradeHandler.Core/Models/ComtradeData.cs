...
public class ComtradeData
{
    ...
    internal ComtradeData(string[] strings, ComtradeConfiguration configuration)
    {
    ...
    internal ComtradeData(IReadOnlyList<byte> bytes, ComtradeConfiguration configuration)
    {
        var samplesCount = configuration.SampleRates[^1].LastSampleNumber;
        Samples = new DataFileSample[samplesCount];
