...
namespace Comtrade.Core
{
    ...
    public class DataFileSample
    {
        ...
        public DataFileSample(byte[] bytes, DataFileType dataFileType, int analogCount, int digitalCount)
        {
            ...
            {
                for (var i = 0; i < analogCount; i++) AnalogValues[i] = BitConverter.ToInt16(bytes, 8 + i * 2);
        ...
        {
            var result =
                new byte[DataFileHandler.GetByteCount(AnalogValues.Length, DigitalValues.Length, dataFileType)];
